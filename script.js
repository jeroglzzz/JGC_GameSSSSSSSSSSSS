// Main Entry Point for Chapa67 Games Hub (Compatible with GitHub Pages and Local Hosting)
...
export function setCategory(cat) {
    ...
    if (hero) {
        hero.classList.toggle('hidden', cat !== 'All' || currentSearch !== '');
    }
}
...
export function handleSearch(val) {
    ...
    const hero = document.getElementById('hero-section');
    if (hero) {
        hero.classList.toggle('hidden', currentSearch !== '' || currentCategory !== 'All');
    }
}
            ...
            <div class="aspect-video bg-gradient-to-br from-slate-900 via-indigo-950/40 to-slate-900 rounded-2xl md:rounded-3xl overflow-hidden border border-slate-800 transition-all duration-300 group-hover:border-indigo-500/60 group-hover:scale-[1.02] group-hover:shadow-[0_0_30px_rgba(99,102,241,0.2)] relative">
                ...
                <div class="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-950/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
                     <div class="w-12 h-12 bg-white text-slate-950 rounded-full flex items-center justify-center shadow-2xl scale-75 group-hover:scale-100 transition-transform duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="none" class="w-5 h-5 ml-0.5"><polygon points="6 3 20 12 6 21 6 3"/></svg>
                     </div>
                </div>
            </div>
            <div class="px-1">
...
export function loadGameIframe(url) {
...
export function launchFeatured() {
    if (games.length > 0) {
        openGame(games[0].id);
    }
}
...
// Global Event Delegation (guarantees click handlers work 100% in all bundled environments)
function setupEventListeners() {
    document.addEventListener('click', (e) => {
        ...
        } else if (action === 'show-gallery') {
            showGallery();
        } else if (action === 'launch-featured') {
            launchFeatured();
...
// Expose on global window object as well
